...
class CLI(object):
    ...
    def parse_arguments(self, args):
        ...
        self.logger.log(
            message=f"Starting manipulator",
            type="info"
        )
        self.manipulator = data_manip.Manipulator(logger=self.logger, dataset_path=self.args.csv, metadata_path=self.args.metadata, target_label=self.args.target, metadata_manip=self.args.manipulate)
        self.logger.log(
            message=f"Created manipulator {self.manipulator}",
            type="ok"
        )
    ...
    def chooseSniffTest(self):
        ...
        for name, sniff in inspect.getmembers(sniffs):
            if inspect.isclass(sniff):
                if sniff_name == name:
                    self.logger.log(
                        message=f"Starting {name} sniff",
                        type="info"
                    )
                    print(self.manipulator)
