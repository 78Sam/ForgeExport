...
class BaseSniff(ABC):
    ...
    def __init__(self, name: str, description: str, type: str, manipulator: data_manip.Manipulator = None, suppress_reformatting: bool = False):
        ...
        Type = Smell Category
        """
        self.logger: Logger = manipulator.logger if manipulator else Logger(rolling_messages=True, interesting_only=False, background=True)
        self.test_name = name
...
class CosineSniff(BaseSniff):
    ...
    def cosine_similarity(self, data, num_points=1000):
        ...
        print(clusters)
        print(cluster_list)
        print(cluster_value_list)

        # Remove any clusters that only have 1 element

        for cluster_id in range(len(cluster_list)):
            if len(data[data["Cluster"] == cluster_id]) < 2:
                data = data[data["Cluster"] != cluster_id]
                cluster_list.remove(cluster_id)

        while 1 in cluster_value_list:
            cluster_value_list.remove(1)

        # How small a number do we need to multiply by to make the largest number less than 1000
        magnitude = 0.1**(len(str(data.max().max())) - 3)

        print(f"Max value: {data.max().max()} mag: {magnitude}")

        # Change the way it loops through as we may have removed a cluster id like '1' in [0, 1, 2]=>[0, 2]
        ...
        for cluster_value in cluster_list:
            ...
            for _ in range(0, num_points):
                ...
                # Calculate cosine distance, and subtract from 1 to calculate cosine similarities
                result = 1 - spatial.distance.cosine(samples.iloc[0].apply(lambda x: x * magnitude), samples.iloc[1].apply(lambda x: x * magnitude))
...
class PortSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(PortSniff, self).__init__(
            "Port Test",
            "Returns the percentage of flows associated with\
    ...
    def pipeline(self):
        ...
        self.logger.log(
            message=f"Writing results for portsniff",
            type="ok"
        )
...
class RollingImportancesSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(RollingImportancesSniff, self).__init__(
            "Rolling Importances Test", 
...
class DropRollingImportancesSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(DropRollingImportancesSniff, self).__init__(
            "Rolling Importances With Drop Test", 
...
class SingleFeatureEfficacySniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        #! How on earth does this work? No commas or anything
        description = (
            "Single Feature Efficacy Test - Orders features according to mutual information "
            "and trains a random forest for classification on a single feature. "
            "Outputs JSON file with F1 score for each feature"
        )
        super(SingleFeatureEfficacySniff, self).__init__(
            "Single Feature Efficacy Test",
            description,
            "Simulation Artefacts",
            manipulator
        )
...
class CorrelationSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(CorrelationSniff, self).__init__(
            "Correlation Test",
...
class SimpleAdversarialSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(SimpleAdversarialSniff, self).__init__(
            "Unguided Adversarial Test",
...
class TimeBehaviourSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(TimeBehaviourSniff, self).__init__(
            "Time vs Behaviour Plots",
...
class FlowRateSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(FlowRateSniff, self).__init__(
            "Flow Rate Test",
...
class RollingRatioSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(RollingRatioSniff, self).__init__(
            "Rolling Ratio Test",
...
class RollingMetricSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(RollingMetricSniff, self).__init__(
            "Rolling Metric Test",
...
class SiameseSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(SiameseSniff, self).__init__(
            "Simese Network",
            "",
            "Data Complexity",
            manipulator=manipulator
...
class CompleteSniff(BaseSniff):
    def __init__(self, manipulator: data_manip.Manipulator):
        super(CompleteSniff, self).__init__(
            "Complete",
            "Run all tests",
            "Other"
        )
